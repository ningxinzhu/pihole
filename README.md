...
- Run `sudo nano /etc/pihole/whitelist.txt` so that you can add the following domains to the whitelist (the first three MUST be added if you ever want to have Internet connectivity on a Windows device ever again):
```
msftncsi.com
*.msftncsi.com
www.msftncsi.com
...
mp.weixin.qq.com
```
- The good news is that you can easily add domains to the whitelist in the Pi-hole web interface, but you can't add more than one domain at once there, so it's good to know how to do it in Terminal for the initial configuration.
